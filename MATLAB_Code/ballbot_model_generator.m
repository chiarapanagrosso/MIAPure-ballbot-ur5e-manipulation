...
% System Parameters
syms m_U I_Uy I_Ux l_U m_S I_S r_S g h_app

% --- ADDITION: External Wrench from Manipulator ---
syms F_px F_py F_pz      % External forces along body axes 
syms tau_px tau_py       % External torques about body axes (roll, pitch)
...
% The generalized force Q_theta_y is the sum of moments about the y-axis.
% A positive external torque/force that causes a positive (forward) theta_y enters with a + sign.
external_moment_y = +tau_py + F_px * h_app; % This is the moment from the manipulator
Q_theta_y = -tau_y + external_moment_y;      % -tau_y is the motor's REACTION torque
...
% The generalized force for phi_y is the TORQUE that the external force
% exerts with respect to the ground contact point.
% It has a lever arm r_S and its effectiveness depends on the tilt angle th_y.
external_torque_on_phi_y = (F_px * cos(th_y) - F_pz * sin(th_y)) * r_S;
Q_phi_y = tau_y - b_phi * diff(phi_y,t) + external_torque_on_phi_y;
...
% Euler-Lagrange Equations for X-Z Plane
% Eq1 for theta_x
dLd_dot_theta_x = diff(L_x, diff(theta_x,t));
ddt_dLd_dot_theta_x = diff(dLd_dot_theta_x, t);
dLd_theta_x = diff(L_x, theta_x);

% Convention: A positive theta_x is a roll to the left.
% A positive external force F_py (to the left) will cause a positive roll.
external_moment_x = +tau_px + F_py * h_app;
Q_theta_x = -tau_x + external_moment_x;
...
external_torque_on_phi_x = (F_py * cos(th_x) - F_pz * sin(th_x)) * r_S;
Q_phi_x = tau_x - b_phi * diff(phi_x,t) + external_torque_on_phi_x;
...
input_vars_y = [tau_y];
param_vars = [m_U, I_Uy, l_U, m_S, I_S, r_S, g, b_phi, h_app]; % Added h_app here
state_vars_x = [th_x, dot_theta_x, dot_phi_x];
input_vars_x = [tau_x];
param_vars_roll = [m_U, I_Ux, l_U, m_S, I_S, r_S, g, b_phi, h_app]; % For roll dynamics
...
matlabFunction(ddot_theta_y_expr, ...
               'File', 'autogen_ddot_theta_y_exforce', ...
...
matlabFunction(ddot_phi_y_expr, ...
               'File', 'autogen_ddot_phi_y_exforce', ...
               'Vars', {state_vars_y, input_vars_y, param_vars, external_vars_y}, ...
               'Outputs', {'ddot_phi_y'});

% Generate function for ddot_theta_x
matlabFunction(ddot_theta_x_expr, ...
               'File', 'autogen_ddot_theta_x_exforce', ...
               'Vars', {state_vars_x, input_vars_x, param_vars_roll, external_vars_x}, ...
               'Outputs', {'ddot_theta_x'});
               
% Generate function for ddot_phi_x
matlabFunction(ddot_phi_x_expr, ...
               'File', 'autogen_ddot_phi_x_exforce', ...
               'Vars', {state_vars_x, input_vars_x, param_vars_roll, external_vars_x}, ...
               'Outputs', {'ddot_phi_x'});
